...
public:
    ...
    {
        return GKEY_UID | 0x8000 | gkey;
    } // 'LG' | mouse bit | mode 0 | gkey
    static constexpr uint32_t PackGKeysSymbolKeyboard(int gkey, int mode)
    {
        return GKEY_UID | (mode << 8) | gkey;
    } // 'LG' | mode | gkey
...
inline bool IsForeground() { return *(BOOL*)0x8D621C != FALSE; }
    ...
    if (numRIDevices > 0)
    {
        ...
        for (size_t i = 0; i < numRIDevices; ++i)
        {
            if (pDevices[i].dwType == RIM_TYPEMOUSE)
            {
                ...
                if (!mouseCaps->lmb)
                    mouseCaps->lmb = deviceInfo.mouse.dwNumberOfButtons >= 1;
                if (!mouseCaps->rmb)
                    mouseCaps->rmb = deviceInfo.mouse.dwNumberOfButtons >= 2;
                // Assume MMB is scroll - seems like there's no obvious way to check for the presence of scroll wheel so
                // this has to suffice
                if (!mouseCaps->mmb)
                    mouseCaps->mmb = mouseCaps->wheelDown = mouseCaps->wheelUp =
                        deviceInfo.mouse.dwNumberOfButtons >= 3;
                if (!mouseCaps->bmx1)
                    mouseCaps->bmx1 = deviceInfo.mouse.dwNumberOfButtons >= 4;
                if (!mouseCaps->bmx2)
                    mouseCaps->bmx2 = deviceInfo.mouse.dwNumberOfButtons >= 5;
            }
        }
        delete[] pDevices;
    }
...
const char* __stdcall GetMXB1Text(const char*) { return "MOUSE BTN 4"; }

const char* __stdcall GetMXB2Text(const char*) { return "MOUSE BTN 5"; }
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
    ...
    if (fdwReason == DLL_PROCESS_ATTACH)
    {
        ...
        if (*(DWORD*)DynBaseAddress(0x82457C) == 0x94BF || *(DWORD*)DynBaseAddress(0x8245BC) == 0x94BF)
            Patch_SA_10();
        // else if (*(DWORD*)DynBaseAddress(0x8252FC) == 0x94BF || *(DWORD*)DynBaseAddress(0x82533C) == 0x94BF)
        // Patch_SA_11();  else if (*(DWORD*)DynBaseAddress(0x85EC4A) == 0x94BF) Patch_SA_Steam();
...
BOOL RawMouse_UsesGKeys() { return bUsesGKeys ? TRUE : FALSE; }

const CGKeysControllerState* RawMouse_GetNewGKeysState() { return CPad::GetNewGKeysState(); }

const CGKeysControllerState* RawMouse_GetOldGKeysState() { return CPad::GetOldGKeysState(); }
}
